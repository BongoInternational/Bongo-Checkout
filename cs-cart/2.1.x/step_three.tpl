{* $Id: step_three.tpl 11215 2010-11-16 08:37:31Z 2tl $ *}
{if $ship_country=="US"}
<div class="step-container{if $edit}-active{/if}" id="step_three">
	<h2 class="step-title{if $edit}-active{/if}">
		<span class="float-left">{if $profile_fields.B || $profile_fields.S}3{$ship_country}{else}2{/if}.</span>
...
<div class="step-container{if $edit}-active{/if}" id="step_three">
	<h2 class="step-title{if $edit}-active{/if}">
		...
		<a class="title{if $complete && !$edit} cm-ajax{/if}" {if $complete && !$edit}href="{"checkout.checkout?edit_step=step_three&amp;from_step=`$edit_step`"|fn_url}" rev="checkout_steps"{/if}>{assign var="giftcoupon" value='Gift coupon code'}{$giftcoupon}</a>
	</h2>
	...
	<div id="step_three_body" class="step-body{if $edit}-active{/if} {if !$edit && !$complete}hidden{/if}">
		<div class="clear">
			{if $edit}
				...
				{assign var="giftcoupontxt" value="Note: if you don't have coupon code Enter 'dumy' in gift coupon code box"}{$giftcoupontxt}
				{if $edit}
					<div class="buttons-container hidden">
						{include file="buttons/button.tpl" but_name="dispatch[checkout.update_steps]" but_text=$but_text but_id="step_three_but"}
					</div>
