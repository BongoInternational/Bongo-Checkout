{* $Id: step_four.tpl 11506 2010-12-29 13:36:30Z angel $ *}
...
<div class="step-container{if $edit}-active{/if}" id="step_four">
	<h2 class="step-title{if $edit}-active{/if}">
		...
		{if $edit && $cart|fn_allow_place_order}
		...
		<div class="clear">
			{if $edit}
				...
				{if $cart|fn_allow_place_order}
    	            ...
    	            {else} 
        	            <form action="{""|fn_url}" method="post" name="summary_form">
                    {/if}
					<div class="clear">
					...
					{if $cart_agreements || $settings.General.agree_terms_conditions == "Y"}
						...
						<tr valign="top">
							...
							<div class="form-field margin-top">
								...
								<label for="id_accept_terms" class="valign cm-custom (check_agreement)"><input type="checkbox" id="id_accept_terms" name="accept_terms" value="Y" class="checkbox valign" />{$lang.checkout_terms_n_conditions}</label>
								{/hook}
							</div>
							{/if}
							{if $cart_agreements}
							<div class="form-field">
								...
								<label for="product_agreements" class="valign cm-custom (check_agreement)"><input type="checkbox" id="product_agreements" name="agreements[]" value="Y" class="valign checkbox" />{$lang.checkout_edp_terms_n_conditions}</label>{include file="buttons/button.tpl" but_text=$lang.license_agreement but_role="text" but_id="sw_elm_agreements" but_meta="cm-combination"}
								{/hook}
