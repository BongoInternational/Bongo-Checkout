/*		
	...
	1.6 - Another fix to resolve the recalculation issue.
	1.7 - Resolved issue with users who are already logged in and have a saved location.
*/
...
function getCustInfo() {
	...
	if (!custInfo['country']) { 
		...
		if (!custInfo['country']) { custInfo['country'] = 'United States';} 
	}
	...
	if (!custInfo['postalCode']) {
		// old format
		custInfo['postalCode'] = $b('form[name="form"] select[name="Precalc_PostalCode"]').val();
		if (!custInfo['postalCode']) { custInfo['postalCode'] = ''; }
	}
	...
	if (!custInfo['state']) {
		// old format
		custInfo['state'] = $b('form[name="form"] input[name="Precalc_State"]').val();
		if (!custInfo['state']) { custInfo['state'] = ''; }
	}
...
function isMerchantSupportedCountry(selected_country) {
	...
	//selected_country = custInfo['country'];
	...
	if (!supported_countries) { if (selected_country != 'United States') { return false; } }
...
function showShippingInfo() {
	...
	if (document.getElementById('intlShippingInfo')){
		...
		$b('form[name="form"] input[name="btnRecalculate"]').parent().hide();
	}
}
...
function hideShippingInfo() {
	...
	if (document.getElementById('intlShippingInfo')){
		...
		$b('form[name="form"] input[name="btnRecalculate"]').parent().show();
	}
}

function appendShippingNotice() {
	if (document.getElementById('intlShippingInfo')) {
...
function intlShippingInfo() {
	...
	if (!document.getElementById('intlShippingInfo')) {
		...
		if (!document.getElementById('intlShippingInfo')) {
			try
			{
				...
				var loc = $b('#v65-cart-shipping-details tbody tr:nth-child(3) td').html().split(',');

				if (!isMerchantSupportedCountry(loc[0]))
				{
					$b('form[name="form"] select[name="ShippingSpeedChoice"]').attr('disabled','disabled');
					$b('.v65-cart-shipping-details-parent-cell').append('<div id="intlShippingInfo"><p><b>International Customers!</b><br />'+message+'</p></div>');
					
					appendShippingNotice();
					
					
				} else {
					$b('form[name="form"] select[name="ShippingSpeedChoice"]').removeAttr('disabled');
				}
			}
			catch(err)
			{
    ...
    // Check whether to display shipping box
	...
	if (loc) {
		if (isMerchantSupportedCountry(loc[0])) { showShippingInfo(); } else { hideShippingInfo(); }
	}
...
function internationalOrder() {	
	...
	if (custInfo['postalCode']) {
		$b('form[name="internationalForm"]').append('<input style="display:none;" type="text" name="CUST_ZIP" value="'+custInfo['postalCode']+'" /> ');
	}
...
function isInternationalCustomer() {
    var loc = $b('#v65-cart-shipping-details tbody tr:nth-child(3) td').html().split(',');
	var custInfo = getCustInfo();
	
	if ($b('select[name="ShippingSpeedChoice"]').is(':visible')) {
		if (loc) {
			if (!isMerchantSupportedCountry(loc[0])) { return true; }
		}
	} else {
		if (!isMerchantSupportedCountry(custInfo['country'])) { return true; }
	}
