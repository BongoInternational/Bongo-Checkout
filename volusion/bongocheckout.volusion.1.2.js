...
function getItemIDs() {
    ...
    urls = $('form[name="form"] td a[href^="ProductDetails.asp"]');
...
function getItemNames() {
	names = new Array();
	elements = $('form[name="form"] td a[href^="ProductDetails.asp"] b');
...
function getItemQuantities() {
	...
	elements = $('form[name="form"] td input[name^="Quantity"]');
...
function getItemPrices() {
	price = '';
	prices = new Array();
	elements = $('form[name="form"] td div font:contains("$")').not(':odd');
...
function getCustInfo() {
	...
	custInfo['country'] = $('form[name="form"] select[name="ShipCountry"]').val();
	custInfo['country_2letteriso'] = countryCode($('form[name="form"] select[name="ShipCountry"]').val());
	custInfo['postalCode'] = $('form[name="form"] input[name="ShipPostalCode"]').val();
	custInfo['state'] = $('form[name="form"] input[name="ShipState"]').val();
...
function isMerchantSupportedCountry(selected_country) {
	...
	var supported_countries = $('form[name="internationalForm"] input[name="MERCHANT_SUPPORTED_COUNTRIES"]').val().split(',');
	...
	for (var i=0; i < supported_countries.length; i++) {
		//$('#debug').append('Supported Country == ' + supported_countries[i] +'<br />');
...
function showShippingInfo() {
	...
	$('#intlShippingInfo').css("display", "block");
	$('#intlShippingInfo').css("visibility", "visible");

	// Hide 'Recalculate' button
	$('form[name="form"] input[name="btnRecalculate"]').css("visibility", "hidden");
}

function hideShippingInfo() {
	// Hide message for US customers
	$('#intlShippingInfo').css("display", "none");
	$('#intlShippingInfo').css("visibility", "hidden");

	// Show 'Recalculate' button
	$('form[name="form"] input[name="btnRecalculate"]').css("visibility", "visible");
}
...
function intlShippingInfo() {
	...
	// Add a hidden box to display the message
	$('form[name="form"] select[name="ShipCountry"]').parent().parent().parent().parent().parent().append('<div id="intlShippingInfo"><p><b>International Customers!</b><br />Shipping duties, taxes, and fees will be assessed on the next page.</p></div>');
	
	$('#intlShippingInfo').css("border", "2px solid rgb(255, 101, 1)");
	$('#intlShippingInfo').css("padding", "5px");
	$('#intlShippingInfo').css("margin", "10px 0 10px 0");
	$('#intlShippingInfo').css("display", "none");
    $('#intlShippingInfo').css("visbility", "hidden");
    $('#intlShippingInfo').css("width", "280px");

    init_intlShippingInfo();

	$('form[name="form"] select[name="ShipCountry"]').change( function() {
		if ( isMerchantSupportedCountry($(this).val()) == false ) { showShippingInfo(); } else { hideShippingInfo(); }
	});
}
...
function internationalOrder() {	
	...
	$('form[name="internationalForm"]').append('<input type="hidden" name="PARTNER_KEY" value="'+partner_key+'">');
	...
	for (i=0; i < itemIDs.length; i++) {
		$('form[name="internationalForm"]').append('<input type="hidden" name="PRODUCT_ID_'+(i+1)+'" value="'+itemIDs[i]+'" /> ');
		$('form[name="internationalForm"]').append('<input type="hidden" name="PRODUCT_NAME_'+(i+1)+'" value="'+itemNames[i]+'" /> ');
		$('form[name="internationalForm"]').append('<input type="hidden" name="PRODUCT_PRICE_'+(i+1)+'" value="'+itemPrices[i]+'" /> ');
		$('form[name="internationalForm"]').append('<input type="hidden" name="PRODUCT_Q_'+(i+1)+'" value="'+itemQuantities[i]+'" /> ');
...
function overrideCheckoutButtons() {
    ...
    if ($('form[name="form"] select[name="ShippingSpeedChoice"]').val() == undefined ) {
	
		// Override Proceed to Checkout
        $('form[name="Proceed_To_Checkout_Form"] input[name="btn_checkout_guest"]').click(function(event) {
            event.preventDefault();
            var custInfo = getCustInfo();
            if (isMerchantSupportedCountry(custInfo['country']) == false ) { internationalOrder(); } else { $('form[name="form"]').submit(); }
        });
		
		$('form[name="Proceed_To_Checkout_Form"] input[name="btn_checkout_login"]').click(function(event) {
            event.preventDefault();
            var custInfo = getCustInfo();
            if (isMerchantSupportedCountry(custInfo['country'] ) == false) { internationalOrder(); } else { $('form[name="form"]').submit(); }
        });
