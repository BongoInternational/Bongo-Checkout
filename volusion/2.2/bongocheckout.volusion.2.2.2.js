/*		
	...
	CHANGELOG:
	---
	2.2.2	- Changed how prices are being filtered, now uses a regex
			- Fixed issue with border color not being applied
			- New button image
...
var BongoCheckout = {
	...
	message_abovebutton: '',
	message_abovebutton_border_color: '#fff',
	message_abovebutton_bg_color: '#fff',
	message_belowshipping: 'Click the orange button below. The shipping duties, taxes, and processing will be assessed on the next page.',
	message_belowshipping_border_color: '#fff',
	message_belowshipping_bg_color: '#fff',
	button_image: 'http://public.bongocheckout.com/Bongo_Checkout_Button.png',
...
*/
	...
	getSubItemID: function(subitem_str) {
		var txt=subitem_str;

		var re1='(\' \\+ escape\\(\')';	// Single Quote String 1
	...
	getItemIDs: function() {
		var real_ids = new Array();
		var ids = new Array();
		var ids_filtered = new Array();
		//urls = jqb('form[name="form"] td a[href^="ProductDetails.asp"]');

		urls = jqb('form[name="form"] td a');
		if (this.debug) { console.log('URLS: ', urls); }
		...
		for (i=0; i < urls.length; i++) {
			...
			if (ids[i].length > 0) {
				if (ids[i][0] == "'") {
					ids[i] = this.getSubItemID(ids[i]);
				}
				
				if (ids[i].length > 0) {
					if (this.debug) {  console.log('ItemID: ', ids[i]); }
					real_ids.push(ids[i]);
				}
			}
		}
	...
	getItemNames: function() {
		...
		for (i=0;i < elements.length; i++) {
			...
			names[i] = elements[i].innerHTML.replace(/(<([^>]+)>)/ig,"");
			
			if (names[i].length > 0) { 
				real_names.push(trim(names[i])); 
				if (this.debug) { console.log('ItemName: ', trim(names[i])); }
			}
	...
	getItemPrices: function() {
		...
		for (i=0; i < elements.length; i++) {
			...
			prices[i] = price.substr(1).replace(/[^\d.]/g,'');
	...
	insertButton: function() {
		var button_html = '<div id="BongoCheckout-AboveButton" style="width:400px; padding:5px; margin:5px; float:right; text-align:center; border:1px solid '+this.message_abovebutton_border_color+'; background:'+this.message_abovebutton_bg_color+'; padding:10px; margin:10px 0;">'+this.message_abovebutton+'<img style="cursor:pointer;" src="'+this.button_image+'" onclick="javascript:document.BongoCheckoutForm.submit()" border="0"/></div>';
		
		// v6.5 Specific
		if (document.getElementById('#primaryArticleText') != null) {
